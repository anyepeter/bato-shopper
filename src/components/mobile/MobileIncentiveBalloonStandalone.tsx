...
export function MobileIncentiveBalloonStandalone({
  ...
  useEffect(() => {
    ...
    console.log('🎈 MobileIncentiveBalloon: localStorage dismissed status:', dismissed);
    if (dismissed === 'true') {
      setIsPermanentlyDismissed(true);
      console.log('🎈 MobileIncentiveBalloon: Permanently dismissed, not showing');
    } else {
      console.log('🎈 MobileIncentiveBalloon: Not dismissed, component will show');
    }
  }, []);
  ...
  useEffect(() => {
    if (currentProduct) {
      ...
      if (newOffer) {
        ...
        console.log('🎈 Updated offer for product:', currentProduct.name, 'New offer:', newOffer.title);
      }
    }
  ...
  useEffect(() => {
    ...
    const showBalloon = () => {
      console.log('🎈 MobileIncentiveBalloon: showBalloon called, isVisible:', isVisible, 'isPermanentlyDismissed:', isPermanentlyDismissed);
      if (!isVisible && currentProduct) {
        ...
        if (newOffer) {
          ...
          console.log('🎈 MobileIncentiveBalloon: Showing balloon with product-specific offer:', newOffer.title, 'for product:', currentProduct.name);
        }
  ...
  const handleAction = () => {
    console.log('🎯 Incentive offer clicked:', currentOffer?.id);
    if (onAction && currentOffer && currentProduct) {
      ...
      console.log('🎯 Sending incentive data:', incentiveData);
  ...
  if (isPermanentlyDismissed || !isVisible || !currentProduct || !currentOffer) {
    console.log('🎈 MobileIncentiveBalloon: Not rendering - isPermanentlyDismissed:', isPermanentlyDismissed, 'isVisible:', isVisible, 'currentProduct:', !!currentProduct, 'currentOffer:', !!currentOffer);
    return null;
  }
  
  console.log('🎈 MobileIncentiveBalloon: Rendering balloon - isVisible:', isVisible, 'currentOffer:', currentOffer.title, 'product:', currentProduct.name);
  ...
  return (
    <AnimatePresence>
      ...
      >
          ...
          >
            ...
            <div className="relative z-10 text-center">
              ...
              <motion.button
                ...
                className="w-full h-10 rounded-2xl font-bold text-white font-body text-sm flex items-center justify-center gap-1 shadow-lg"
                style={{
                  background: currentOffer.gradient,
              ...
              >
                ...
                <span className="text-xs text-white/50 ml-1 font-body">4.9 (2.1k reviews)</span>
              </motion.div>
            </div>
        ...
        {/* Reduced Background Particles for smaller balloon */}
        {[...Array(6)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute text-lg opacity-15 pointer-events-none"
            style={{
              left: `${15 + (i * 12)}%`,
              top: `${25 + (i % 3) * 25}%`,
              color: currentOffer.color
