...
export function MobileCartPageWithIncentives({
  cartItems,
  onNavigateBack,
  onUpdateQuantity,
  onRemoveItem,
  ...
  // Calculate total with incentive discounts
  const totalAmount = cartItems.reduce((total, item) => {
    const itemPrice = item.incentive ? item.incentive.discountedPrice : item.product.price;
    return total + (itemPrice * item.quantity);
  }, 0);

  // Calculate total savings from incentives
  const totalSavings = cartItems.reduce((savings, item) => {
    if (item.incentive) {
      const savedPerItem = item.incentive.originalPrice - item.incentive.discountedPrice;
      return savings + (savedPerItem * item.quantity);
    }
    return savings;
  }, 0);
              ...
              {cartItems.map((item, index) => (
                <motion.div
                  ...
                  style={{
                    ...
                    border: item.incentive ? '1px solid rgba(34, 197, 94, 0.3)' : '1px solid rgba(255,255,255,0.1)'
                  }}
                >
                  {/* Incentive Glow Effect */}
                  {item.incentive && (
                    <div 
                    ...
                    <div className="flex-1 ml-4">
                      ...
                      {item.incentive && (
                        <div className="flex items-center gap-1 mb-2">
                          <span className="text-xs px-2 py-1 font-medium" style={{ 
                            ...
                            🎉 {item.incentive.offerTitle} - Save ${((item.incentive.originalPrice - item.incentive.discountedPrice) * item.quantity).toFixed(2)}
                          </span>
                        </div>
                      )}
                            ...
                            {item.product.colors.map((color) => (
                              <motion.button
                                ...
                                className={`w-5 h-5 rounded-full transition-all ${
                                  item.color === color
                                    ? 'border-2 border-[#5825efff] shadow-lg'
                                    : 'border-2 border-white/30 hover:border-white/60'
                                }`}
                                style={{ 
                                  backgroundColor: getColorCode(color),
                                  ...(item.color === color ? { 
                                    boxShadow: '0 0 0 2px rgba(88, 37, 239, 0.3)',
                                    transform: 'scale(1)'
                                  } : {})
                                }}
                                title={color}
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col gap-1">
                          {/* Show incentive discount if applicable */}
                          {item.incentive ? (
                            <>
                              <div className="flex items-center gap-2">
                                <span className="text-green-400 font-heading text-lg">
                                  ${(item.incentive.discountedPrice * item.quantity).toFixed(2)}
                                </span>
                                <span className="text-white/50 text-sm line-through">
                                  ${(item.incentive.originalPrice * item.quantity).toFixed(2)}
                                </span>
                              </div>
                            </>
                          ) : (
      ...
      {cartItems.length > 0 && (
        ...
        >
          <div className="mb-4 p-4 rounded-xl" style={{ backgroundColor: 'rgba(255,255,255,0.1)' }}>
            <div className="flex justify-between items-center mb-2" style={{ backgroundColor: 'transparent' }}>
            ...
            {/* Show savings if any incentives are applied */}
            {totalSavings > 0 && (
              <div className="flex justify-between items-center mb-2" style={{ backgroundColor: 'transparent' }}>
                <span className="text-green-400 font-body text-sm">🎉 Incentive Savings</span>
                <span className="text-green-400 font-body text-sm">-${totalSavings.toFixed(2)}</span>
              </div>
            )}
