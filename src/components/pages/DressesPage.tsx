...
export function DressesPage({
  ...
  // Filter to show only dresses
  const dresses = filteredProducts.filter(product => product.category.toLowerCase() === 'dresses');
  ...
  const categoryData = [
    { id: 'all', name: 'All Products', count: filteredProducts.length, icon: '🌟', isActive: selectedCategory === 'all' },
    { id: 'dresses', name: 'Dresses', count: filteredProducts.filter(p => p.category.toLowerCase() === 'dresses').length, icon: '👗', isActive: selectedCategory === 'dresses' },
    { id: 'tops', name: 'Tops', count: filteredProducts.filter(p => p.category.toLowerCase() === 'tops').length, icon: '👚', isActive: selectedCategory === 'tops' },
    { id: 'sets', name: 'Sets', count: filteredProducts.filter(p => p.category.toLowerCase() === 'sets').length, icon: '👕', isActive: selectedCategory === 'sets' },
    { id: 'traditional', name: 'Traditional', count: filteredProducts.filter(p => p.category.toLowerCase() === 'traditional').length, icon: '🎭', isActive: selectedCategory === 'traditional' },
    { id: 'accessories', name: 'Accessories', count: filteredProducts.filter(p => p.category.toLowerCase() === 'accessories').length, icon: '💎', isActive: selectedCategory === 'accessories' }
  ];
        ...
        <div className="shop-pagination-content">
          <div className="shop-pagination-info">
            <div className="shop-pagination-results">
              Showing {startItem}-{endItem} of {totalProducts} beautiful dresses
            </div>
        ...
        >
          <div className="shop-panel-card shop-search-filter-card">
            ...
            >
              ...
              <Input
                type="text"
                placeholder="Search exquisite dresses..."
                value={searchQuery}
        ...
        >
          ...
          >
            ...
            <motion.p
              ...
              transition={{ duration: 0.6, delay: 0.8 }}
            >
              Discover stunning African-inspired dresses that celebrate culture and elegance
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          >
            {paginatedProducts.length === 0 ? (
              ...
              >
                ...
                <h3 className="shop-empty-state-title">No dresses found</h3>
                <p className="shop-empty-state-description">Try adjusting your search or filter criteria to discover more beautiful dresses</p>
              </motion.div>
            ) : (
              <motion.div 
