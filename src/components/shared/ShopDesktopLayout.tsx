...
import { Search, Filter, SlidersHorizontal, Star, TrendingUp, Clock, Users, ChevronLeft, ChevronRight, Shield, Settings, UserCog, Sparkles, ArrowRight } from "lucide-react";
...
interface ShopDesktopLayoutProps {
  title: string;
...
export function ShopDesktopLayout({
  title,
  filteredProducts,
  allFilteredProducts,
  filterCategory,
  ...
  // Helper function to get icon size for categories (4x larger in desktop/tablet)
  const getCategoryIconSize = () => {
    if (isMobile) return '24px'; // Keep mobile size unchanged
    return '96px'; // 4x larger for desktop and tablet (24px * 4 = 96px)
  };
  ...
  return (
    ...
    >
      <div className={layoutClasses.container}>
        {/* 🔥 LEFT PANEL - Categories, Brands, Collections (EXACT COPY FROM HOMEPAGE) */}
        <div className={layoutClasses.leftPanel}>
          ...
          >
            ...
            <div className="flex mb-6 gap-1 p-1 rounded" style={{ backgroundColor: 'var(--light-gray)' }}>
              {[
                { key: 'categories', label: 'Categories', icon: 'apps' },
                ...
                <button
                  ...
                  style={{
                    backgroundColor: activeLeftTab === tab.key ? 'var(--primary-blue)' : 'transparent'
                  }}
                >
            ...
            {activeLeftTab === 'categories' && (
              <div className="space-y-4">
                <h3 className="font-heading font-medium" style={{ color: 'var(--primary-blue)' }}>
                  Shop by Category
                </h3>
                ...
                <div className="space-y-3">
                  {brandData.map((brand, index) => (
                    <motion.div
                      key={index}
                      whileHover={{ scale: 1.02 }}
                      className="p-3 rounded-lg border cursor-pointer transition-all duration-300 hover:shadow-sm"
                      style={{
                        backgroundColor: 'var(--light-gray)',
                        borderColor: brand.featured ? 'var(--primary-blue)' : 'transparent',
                        borderRadius: 'var(--radius-md)'
                      }}
                    >
                      ...
                      <div className="flex items-center justify-between text-sm text-gray-600">
                        <span className="font-body">{brand.products} products</span>
                        <div className="flex items-center gap-1">
                          <Star className="h-3 w-3 fill-yellow-400 text-yellow-400" />
                          <span className="font-body">{brand.rating}</span>
                        </div>
                      </div>
                    </motion.div>
                  ))}
            ...
            {activeLeftTab === 'collections' && (
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {['Summer Essentials', 'Ankara Classics', 'Modern Traditional', 'Wedding Collection'].map((collection, index) => (
                    <motion.div
                      key={index}
                      whileHover={{ scale: 1.02 }}
                      className="p-3 rounded-lg cursor-pointer transition-all duration-300 hover:shadow-sm"
                      style={{
                        backgroundColor: 'var(--light-gray)',
                        borderRadius: 'var(--radius-md)'
                      }}
                    >
                      <div className="font-medium font-body mb-1" style={{ color: 'var(--primary-blue)' }}>
                        {collection}
                      </div>
                      <div className="text-sm text-gray-600 font-body">
        ...
        {/* 🔥 MIDDLE PANEL - Main Content (EXACT COPY FROM HOMEPAGE) */}
        <div className={layoutClasses.middlePanel}>
          ...
          >
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <h1 className="font-heading font-medium" style={{ color: 'var(--primary-blue)' }}>
                  {title}
                </h1>
                <div className="text-sm text-gray-600 font-body">
        ...
        {/* 🔥 RIGHT PANEL - Trending & Hot (EXACT COPY FROM HOMEPAGE) */}
